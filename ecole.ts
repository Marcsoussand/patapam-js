...
const ecolePanel        = document.getElementById("ecolePanel")        as HTMLDivElement;
const ecolePanelContent = document.getElementById("ecolePanelContent") as HTMLDivElement;
const ecolePanelClose   = document.getElementById("ecolePanelClose")   as HTMLButtonElement;

// ── État ──────────────────────────────────────────────────
type NodeStatus = "locked" | "unlocked" | "done";
interface NodeState { status: NodeStatus; stars: number; }

const ECOLE_STORAGE_KEY = "patapam_ecole_nodes";

function defaultNodeStates(): Record<number, NodeState> {
    return {
...
function loadNodeStates(): Record<number, NodeState> {
    try {
        const raw = localStorage.getItem(ECOLE_STORAGE_KEY);
        if (raw) return JSON.parse(raw) as Record<number, NodeState>;
    } catch {}
    return defaultNodeStates();
}

function saveNodeStates(): void {
    localStorage.setItem(ECOLE_STORAGE_KEY, JSON.stringify(nodeStates));
}

const nodeStates: Record<number, NodeState> = loadNodeStates();

let currentLevel  = 1;
let currentGrade: "cp" | "ce2" = "cp";
...
function wrongChoices(correct: number, min: number, max: number): number[] {
    ...
    while (set.size < 2 && attempts < 100) {
        ...
        if (c !== correct && c >= min && c <= max) set.add(c);
    }
    for (let d = 1; set.size < 2; d++) {
        if (correct + d <= max) set.add(correct + d);
        if (set.size < 2 && correct - d >= min) set.add(correct - d);
    }
    return [...set].slice(0, 2);
}

// ── Génération de questions ───────────────────────────────
interface Question { text: string; visual?: string; answer: number; choices: number[]; isLtr?: boolean; }

function makeQuestion(level: number, grade: "cp" | "ce2"): Question {
    if (grade === "cp") {
        switch (level) {
            case 1: {
                const emojis = ["🍎","🌟","🐠","🌸","🎈","🦋","🍄","🐥","🍉","⭐"];
        ...
        switch (level) {
            ...
            default: {
                if (rnd(0, 1) === 0) {
                    const a = rnd(20, 80); const b = rnd(5, a - 5);
                    const ans = a - b;
                    const txt = t(
                        `Patapam a ${a} pommes. Il en mange ${b}. Combien en reste-t-il ?`,
                        `Patapam has ${a} apples. He eats ${b}. How many are left?`,
                        `לפטפם יש ${a} תפוחים. הוא אוכל ${b}. כמה נשארו?`
                    );
                    return { text: txt, answer: ans, choices: [ans, ...wrongChoices(ans, 0, 80)].sort(() => Math.random() - 0.5) };
                } else {
                    const g = rnd(2, 9); const e = [2, 5, 10][rnd(0, 2)];
                    const ans = g * e;
                    const txt = t(
                        `Bobby a ${g} groupes de ${e} ballons. Combien en tout ?`,
                        `Bobby has ${g} groups of ${e} balloons. How many in total?`,
                        `לבובי יש ${g} קבוצות של ${e} בלונים. כמה בסך הכל?`
                    );
...
function renderNodes(): void {
    [1, 2, 3].forEach(level => {
        const btn   = document.getElementById(`ecoleNode${level}`)  as HTMLButtonElement;
        const stars = document.getElementById(`ecoleStars${level}`) as HTMLDivElement;
...
function showGradeSetup(): void {
    ...
    `;
    ecolePanelContent.querySelectorAll<HTMLButtonElement>(".ecole-grade-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            currentGrade = (btn.dataset.grade ?? "cp") as "cp" | "ce2";
...
function playQuestions(level: number, grade: "cp" | "ce2", index: number, errors: number): void {
    currentScreen = null;
    ...
    ecolePanelContent.querySelectorAll<HTMLButtonElement>(".ecole-choice-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            if (!isCorrect) {
                ecolePanelContent.querySelectorAll<HTMLButtonElement>(".ecole-choice-btn").forEach(b => {
                    if (parseInt(b.dataset.val!) === q.answer) b.classList.add("is-correct");
                });
            }
...
// ── Écran de résultat ─────────────────────────────────────
function showLevelResult(level: number, stars: number): void {
    if (stars > 0) {
        if (stars > nodeStates[level].stars) nodeStates[level].stars = stars;
        nodeStates[level].status = "done";
        if (level < 3) nodeStates[level + 1].status = "unlocked";
    }
    renderNodes();
    saveNodeStates();
...
[1, 2, 3].forEach(level => {
    document.getElementById(`ecoleNode${level}`)?.addEventListener("click", () => {
        if (nodeStates[level].status !== "locked") openLevel(level);
    });
});
...
document.getElementById("ecoleResetBtn")?.addEventListener("click", () => {
    const msg = t(
        "Recommencer à zéro ? Toutes les étoiles seront effacées.",
        "Start over? All stars will be erased.",
        "להתחיל מחדש? כל הכוכבים יימחקו."
    );
    if (confirm(msg)) {
...
new MutationObserver(() => {
    if (ecolePanel.classList.contains("is-open") && currentScreen) currentScreen();
