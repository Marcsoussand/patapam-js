...
// ── Popup Glenn Doman ─────────────────────────────────────
const DOMAN_CONTENT: Record<string, { title: string; dir: string; html: string }> = {
    fr: {
...
function showDomanPopup(): void {
    ...
    const titleEl = overlay.querySelector<HTMLElement>(".doman-popup__header h3")!;
    const bodyEl  = overlay.querySelector<HTMLElement>(".doman-popup__body")!;

    function renderDomanContent(): void {
        const lang = getLang();
